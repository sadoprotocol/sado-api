...
export async function get(address: string): Promise<any> {
  ...
  async function filter(o: any, r: any, t: any) {
    if (typeof r === "object") {
      if (r.type === "buy") {
        r.buy = true;
        r.sell = false;
      ...
      if (t === "order") {
        ...
        if (offer_count === orderbook.cids.offers.length && order_count === orderbook.cids.orders.length) {
  ...
  async function finalFilter(order_book: any) {
    for (let i = 0; i < order_book.orders.length; i++) {
      const vArg = order_book.orders[i].location.split(":");
      const txid = vArg[0];
    ...
    for (let i = 0; i < order_book.offers.length; i++) {
      const vArg = order_book.offers[i].order.location.split(":");
      const txid = vArg[0];
      const vout_n = parseInt(vArg[1]);
  ...
  const txs = await lookup.transactions(address);
  ...
  if (typeof txs === "object" && txs.length > 0) {
    for (let i = 0; i < txs.length; i++) {
      for (let t = 0; t < txs[i].vout.length; t++) {
        if (
          ...
          const vs = txs[i].vout[t].scriptPubKey.utf8.split("=");
          const ids = vs[1].split(":");
          const type = ids[0];
          const cid = ids[1];
    ...
    const order_cids = orderbook.cids.orders;
    const offer_cids = orderbook.cids.offers;

    const redisKey = `/${network}/sado/get/${address}/orders-offers/${order_cids.length}-${offer_cids.length}`;
    ...
    if (order_cids.length > 0) {
      for (let od = 0; od < order_cids.length; od++) {
        const response = await infura.get(order_cids[od]);
        ...
        if (
          ...
          const owner = await getOwner(response.location);
    ...
    if (offer_cids.length > 0) {
      for (let off = 0; off < offer_cids.length; off++) {
        const response = await infura.get(offer_cids[off]);
        ...
        if (
          ...
          const temp_tx = bitcoinjs.Transaction.fromHex(response.offer);
          ...
          if (any_signatures) {
            const origin = await infura.get(response.origin);
